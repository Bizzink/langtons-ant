...
class Tile:
    def __init__(self, x, y, rules, batch, scale=20, group=None, show = False):
    ...
    def update(self):
        ...
        direction = self._rules[self._rule_ind]["direction"]

        self._rule_ind += 1
        self._rule_ind %= len(self._rules)

        self._activated = True
        self.show()

        self._set_colour(self._rules[self._rule_ind]["colour"])
...
class Grid:
    def __init__(self, size, rules, batch, window, scale=20):
        self._origin = [(window.width - (size[0] * scale)) // 2, (window.height - (size[1] * scale)) // 2]

        self._tiles = [
            [Tile((col * scale) + self._origin[0], (row * scale) + self._origin[1], rules, batch, scale = scale,
    ...
    def add_dim(self, side):
        ...
        if side == 0:  # top
            ...
            self._tiles.append([Tile((col * self._scale) + self._origin[0],
                                     (self._size[1] * self._scale) + self._origin[1], self._rules, self._batch,
            ...
            for row_ind, row in enumerate(self._tiles):
                row.append(Tile((self._size[0] * self._scale) + self._origin[0],
                                (row_ind * self._scale) + self._origin[1], self._rules, self._batch,
            ...
            self._tiles.insert(0, ([Tile((col * self._scale) + self._origin[0],
                                         self._origin[1], self._rules, self._batch, scale=self._scale,
            ...
            for row_ind, row in enumerate(self._tiles):
                row.insert(0, (Tile(self._origin[0], (row_ind * self._scale) + self._origin[1], self._rules,
